...
qx.Class.define('eyeos.dialogs.FileChooser', {
	...
	members: {
		...
		_browsePath_callback: function(returnData, error) {
			...
			for(var i = 0; i < returnData['files'].length; i++) {
				...
				if (returnData['files'][i]['type'] == 'folder') {
					iconUrl = 'eyeos/extern/images/16x16/mimetypes/inode-directory.png';
				} else {
					iconUrl = 'eyeos/extern/images/16x16/mimetypes/application-x-zerosize.png';
				}
				data[i] = new Array(
		...
		_getWindow: function(title, icon) {
			...
			var parentFolderButton = new qx.ui.form.Button(null, 'eyeos/extern/images/16x16/actions/go-up.png');
			parentFolderButton.addListener('execute', this._browseParentPath, this);
			firstBox.add(parentFolderButton);
			var homeButton = new qx.ui.form.Button(null, 'eyeos/extern/images/16x16/places/user-home.png');
			homeButton.addListener('execute', function(e) {
				this._browsePath('home://');
			}, this);
			firstBox.add(homeButton);
			var newFolderButton = new qx.ui.form.Button(null, 'eyeos/extern/images/16x16/actions/folder-new.png');
			newFolderButton.addListener('execute', this._onNewFolderButtonClick, this);
			firstBox.add(newFolderButton);
			var uploadButton = new qx.ui.form.Button(null, 'eyeos/extern/images/eyefiles/upload.png');
		...
		_onCellClick: function(e) {
			...
			if(row[0] != '<img src="eyeos/extern/images/16x16/mimetypes/inode-directory.png" />') {
		...
		showOpenDialog: function(parentWindow, callback, callbackContext, initialPath) {
			...
			var win = this._getWindow('Open', 'eyeos/extern/images/16x16/actions/document-open.png');
		...
		showSaveDialog: function(parentWindow, callback, callbackContext, initialPath, title) {
			...
			var win = this._getWindow(title, 'eyeos/extern/images/16x16/actions/document-save.png');
		...
		showSaveAttachmentDialog: function(parentWindow, filename, callback, callbackContext, initialPath, title) {
			if(title == null) {
				title = 'Save as';
			}
		...
		showSelectFolderDialog: function(parentWindow, callback, callbackContext, initialPath) {
			...
			var win = this._getWindow('Select folder', 'eyeos/extern/images/16x16/actions/document-open-folder.png');
