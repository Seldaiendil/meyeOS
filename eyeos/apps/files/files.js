...
//					this.getView().showBrowse();
			...
			this._dBusListeners.push(this._dBus.addListener('eyeos_sidebar_urlCreated', function (e) {
				this.getView().updateSideBar('selectedFile', this._selectedElementInfo);
			}, this));
		},

		_addSideBarListeners: function () {
			this.addListener('selectedFile', function (e) {
				var params = {
				...
				this._selectedElementInfo = params;
				this.getView().updateSideBar('selectedFile', params);
				this.getView().fireDataEvent('fileSelectedMenuUpdate', params);
			}, this);

			this.addListener('directoryChanged', function (e) {
				this._selectedElementInfo = e.getData();
				this.getView().updateSideBar('directoryChanged', e.getData());
				this.getView().fireEvent('directoryChangedMenuUpdate');
			}, this);
		},
		...
		_browsePath_callback: function(results, path, addToHistory) {
			...
			// the path ends with eyeSync then is the eyeSync folder
			if (this.getModel().getCurrentPath()[1].substr(-7) == 'eyeSync') {
				this.getView().showEyeSyncBottomBar(true);
			} else {
				this.getView().showEyeSyncBottomBar(false);
			}
			this.getView().showBrowse();
		...
		openFile: function (useEyeRun) {
                    var filesToOpen = this.getView().returnSelected();
                    if( filesToOpen.length == 1 ){
                        var file = filesToOpen[0].getFile().getAbsolutePath();
                        eyeos.openFile(file, this.getApplication().getChecknum(), useEyeRun)
                    }
                    else
                        eyeos.openFiles(filesToOpen, this.getApplication().getChecknum())
		},
		...
		deleteFile: function () {
			...
			if (currentPath.substr(0, 8) != 'share://' && currentPath != 'workgroup://') {
				...
				var optionPane = new eyeos.dialogs.OptionPane(
					"<b>" + tr('Are you sure you want to delete the file?') + "</b>",
					eyeos.dialogs.OptionPane.QUESTION_MESSAGE,
					eyeos.dialogs.OptionPane.OK_CANCEL_OPTION);

					var dialog = optionPane.createDialog(this, tr('Deleting file'), function(result) {
		...
		permissionsFile: function () {
			var selected = this.getView().returnSelected();
			var files = new Array();

			for (var i = 0; i < selected.length; ++i) {
					files.push(selected[i].getFile().getAbsolutePath());
			}
			var currentPath = this.getModel().getCurrentPath()[1];
			var params = {
				cwd: this.getModel().getCurrentPath()[1],
				files: files
			}
			eyeos.execute('permissions', this.getApplication().getChecknum(), [params]);
		},
        openLocalFile: function() {
                var selected = this.getView().returnSelected();
                var filepath = selected[0].getFile().getAbsolutePath();
                var myregexp = new RegExp('[^\:]+:\/\/~'+eyeos.getCurrentUserName()+'/', "gi");
                filepath = filepath.replace(myregexp, "");
                WebRunnerRun('open', filepath);
        }
	}
});
