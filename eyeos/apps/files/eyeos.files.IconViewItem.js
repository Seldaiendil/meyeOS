...
qx.Class.define('eyeos.files.IconViewItem', {
	...
	members: {
		...
		_manager: null,
		_controller: null,
		_decoratorMouseOut: new qx.ui.decoration.RoundBorderBeveled(null, "transparent", 0, 5, 5, 5, 5),
		_decoratorMouseOver: new qx.ui.decoration.RoundBorderBeveled(null, "#D3D3D3", 0, 5, 5, 5, 5),
		_decoratorMouseSelected: new qx.ui.decoration.RoundBorderBeveled(null, "#c1cbd2", 0, 5, 5, 5, 5).set({backgroundColor: '#c1cbd2'}),
		...
		_buildLayout: function () {
			...
			this.addListener('dragstart', function(e) {
				var x = e.getDocumentLeft() + 1;
				var y = e.getDocumentTop() - 30;
				this.clipImage = this._itemImage.clone();
			...
			if (this.getCurrentPath() != 'share:///' && this.getCurrentPath() != 'workgroup:///') {
				...
				if (this.getCurrentPath().substr(0, 8) != 'share://' && this.getCurrentPath() != 'workgroup://') {
					...
					//adding the permissions only if you "right-click" inside a workgroup
					if (this.getCurrentPath().substr(0, 12) == 'workgroup://'){
						var permissions = {
							label: tr('Permissions'),
							//image: 'index.php?extern=images/16x16/apps/system-file-manager.png',
							id: 'permissionsFile()'
						}
						menu = menu.concat(permissions);						
					}
				} else {
					menu = [
						{
				...
				var borderMenu = new qx.ui.decoration.Single(1, 'solid', '#eeeeee');
				this._menu = new eyeos.ui.menu.Menu().set({
				...
				for (var i = 0; i < menu.length; ++i) {
					...
					if (menu[i].id != 'separator') {
						var enabled = true;
						if (this.getFile().getType() == 'folder' && !i) {
							enabled = false;
						}
						item = new eyeos.ui.menu.Button(menu[i].label, menu[i].image);
			...
			//var labelText = name.length > 12 ? name.substr(0, 10) + "..." : name;
		...
		updateImage: function () {
			...
			if (this.getFile().getType() == 'folder') {
				...
				} else if (this.getFile().getName() == 'eyeSync') {
					image = 'index.php?extern=images/48x48/places/folder-remote.png';
		...
		edit: function () {
			...
			this._itemInput.addListener('focusin', function (e) {
				var pointPosition = this.getValue().lastIndexOf('.');
				if(pointPosition > 0){
					this.setTextSelection(0, pointPosition);
				}else{
					this.selectAllText();
				}
			});
