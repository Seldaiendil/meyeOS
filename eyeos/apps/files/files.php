...
abstract class FilesApplication extends EyeosApplicationExecutable {
	...
	public static final function browsePath($params) {
		...
		$content = $leafFolder->listFiles($pattern, $options);
		if($content && is_array($content) && count($content) > 0) {
			foreach($content as $currentFile) {
	...
	private static function simplifyStruct($arrayTree) {
		...
		foreach ($arrayTree as $key => $leaf) {
			if (is_array($leaf)) {
				...
				$newStruct[$key] = array_filter($newStruct[$key], array('FilesApplication','filterJavascript'));
			}
		}
		return $newStruct;
	}
