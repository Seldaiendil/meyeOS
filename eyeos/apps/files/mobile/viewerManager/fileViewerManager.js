...
function fileViewerManager () {
	...
	function createContent (model, page, checknum, callback) {
		...
		if (handler) {
			...
			if(confirm(tr('No Preview available for this file,\ndo you want to download it?'))) {
				//Callback not called, dont want to change the page & make the petition:
				//eyeosmobile.execute('download', checknum, [model.get('absolutepath')]);
				//Manually for avoid the ajax encapsulation
				var param = [model.get('absolutepath')];
				var url = 'index.php?mobile=1&getApplication=download&checknum='+checknum+'&args='+encodeURIComponent($.toJSON(param));
				window.open(url);
			}
		}
	}
	...
	function getIconPath (model, checknum) {
		if (model.get('type') == 'folder') {
			return 'eyeos/extern/images/128x128/mimetypes/inode-directory.png';
		}
		var handler = this.getCorrectHandler(model);
		...
		if (handler != null) {
			...
			// Default Image
			return 'eyeos/extern/images/128x128/mimetypes/application-x-zerosize.png';
		}
	}
}
