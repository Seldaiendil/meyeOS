...
function fvmVideoHandler () {
	...
	function checkRequest (model) {
		var supportedExtensions = ['MOV', '3GP', '3GPP', '3G2', 'MP4', 'MPG', 'MPV', 'AVI', 'OGG', 'OGV', 'WEBM','FLV'];
	...
	function getIconPath (model, checknum) {
		return 'eyeos/extern/images/128x128/mimetypes/application-x-mplayer2.png';
	}
...
//				type = 'type="video/mp4"';
				...
				if(convert) {
					if (Modernizr.video.h264) {
						...
						eyeos.openErrorDialog(tr('This browser doesn\'t support any standard format'));
					}
				} else {
					convertExtension = extension;
				}
...
//					type = 'type=\'video/mp4; codecs="amp4v.20.8, mp4a.40.2"\'';
			...
			content.append('<video controls="controls" preload="auto" autobuffer="autobuffer" width="' + pageWidth + '" poster="'+ poster +'" '+androidfix+'>\n\
								...
								<p>'+tr('This browser doesn\'t support any standard format')+'.</p>\n\
							</video>');
			...
			eyeosmobile.openErrorDialog(tr('This browser doesn\'t support video tag'));
		}
	...
	function getIcon (model) {
		return 'eyeos/extern/images/64x64/mimetypes/application-x-mplayer2.png';
	}
}