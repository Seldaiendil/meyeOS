...
function fvmDocumentHandler() {
	...
	function getIconPath (model, checknum) {
		...
		switch (extension) {
			...
			case 'DOCX':
			case 'ODT':
				return 'eyeos/extern/images/128x128/mimetypes/application-msword.png'
			case 'ODS':
			case 'OTS':
			case 'SXC':
			case 'STC':
			...
			case 'FODS':
			case 'UOS':
				return 'eyeos/extern/images/128x128/mimetypes/application-vnd.ms-excel.png'
			case 'ODP':
			case 'OTP':
			case 'SXI':
			case 'STI':
			...
			case 'FODP':
			case 'UOP':
				return 'eyeos/extern/images/128x128/mimetypes/application-vnd.ms-powerpoint.png'
			case 'PDF':
				return 'eyeos/extern/images/128x128/mimetypes/application-pdf.png'
			case 'EDOC':
				return 'eyeos/extern/images/128x128/mimetypes/application-x-mswrite.png'
		}
	}
		...
		eyeosmobile.callMessage(checknum, 'convertDocumentToImagesHash', {path: model.get('absolutepath')}, function (result) {
			...
			page.getContent().bind('firstPageLoaded', function () {
				if (typeof callback == 'function') {
					...
					//while the first param of the callback isnt jquery object, the var isnt appended to the page
					callback('noAppend');
				}
			});
...
/**	2.-Create Styles	*/
	var newPageStyle =		'display:		none;'+
							...
							'max-width:		98%;'+
							'width:			98%;';
	var imgDivStyle =		'border:		1px solid #B3B3B3;'+
							'clear:			both;';
	...
	var shadowLeftStyle =	'height:		5px;'+
							'width:			2%';
	var shadowCenterStyle = 'display:		inline-block;'+
							'background:	url(eyeos/extern/images/mobile/files/Center.png) repeat-x;'+
							'height:		5px;'+
							'width:			96%';
	var shadowRightStyle =	'height:		5px;'+
							'width:			2%';
...
/**	3.-Create Page	*/
	...
	//Open Page
	$('<img style="' + openPageStyle + '" src="eyeos/extern/images/mobile/files/Text.png" />').appendTo(newPage);

	//Image of the page
	var imageDivPage = $('<div style="' + imgDivStyle + '"></div>').appendTo(newPage);
	...
	var imagePage = $('<img id="'+ imageId +'" style="' + imgStyle + '" src="' + src + '" />').appendTo(imageDivPage);

	//Shadows
	var shadowContent = $('<div style="' + shadowStyle + '"></div>').appendTo(newPage);
	$('<img style="' + shadowLeftStyle  + '" src="eyeos/extern/images/mobile/files/Left.png"/>').appendTo(shadowContent);
	$('<div style="' + shadowCenterStyle + '"/></div>').appendTo(shadowContent);
	$('<img style="' + shadowRightStyle + '" src="eyeos/extern/images/mobile/files/Right.png"/>').appendTo(shadowContent);

	//Loading Image
	var loadingImage = $('<center id="' + loadingImageId + '" class="' + loadingImageClass + '"><img src="eyeos/extern/images/ajax-loader-1.gif" /></center>');
