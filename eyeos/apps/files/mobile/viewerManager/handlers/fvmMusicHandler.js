...
function fvmMusicHandler () {
	...
	function getIconPath (model, checknum) {
		return 'eyeos/extern/images/128x128/mimetypes/audio-x-generic.png';
	}
...
//		if($.mobile.activePage.children('[data-role="content"]').outerHeight() - 30 < maxSize) maxSize = $.mobile.activePage.children('[data-role="content"]').outerHeight();
		...
		var poster = 'eyeos/extern/images/ajax-loader-1.gif';
		var type = '';
		...
		if (Modernizr.audio) {
			try {
				if(eval("Modernizr.audio."+model.get('extension').toLowerCase())) {
					...
					if(Modernizr.audio.mp3) {
						...
						eyeosmobile.openErrorDialog(tr("This browser doesn't support any standard format"));
					}
				}
			} catch (e) {
			...
			eyeosmobile.openErrorDialog(tr('This browser doesn\'t support audio tag'));
		}
	...
	function getIcon (model) {
		return 'eyeos/extern/images/64x64/mimetypes/audio-x-generic.png';
	}
