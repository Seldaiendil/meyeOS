...
qx.Class.define('desktop.actionManager', {
    ...
    members: {
		...
		editFile: function() {
			...
			var aux = file.getChildControl('label').getValue();
			if (aux.indexOf('.') < 0) {
				textbox.setTextSelection(0, aux.length);
			} else {
				textbox.setTextSelection(0, aux.lastIndexOf('.'));
			}
			file._add(textbox);
			textbox.focus();
			...
			var rename = function(e) {
				...
				// valid characters (with ñ and ç and "tildes"-dieresis)
				var exp = /^[0-9a-zA-Z_\.\-ñÑçÇàáèéìïíòóùúü]+$/;
				if (exp.test(newname)) {
					...
					var op = new eyeos.dialogs.OptionPane(
						tr('Error with the filename; contains invalid characters [#$%/;,@+()[]="\'].'),
						eyeos.dialogs.OptionPane.ERROR_MESSAGE);
					op.createDialog(null, tr('Filename error')).open();
					parent.getChildControl('label').show();
				}
			};
