...
class filesystem_attachments extends rcube_plugin
{
    ...
    function upload($args)
    {
		...
		$num = 0;
		/*while(file_exists($temp_dir.'/'.$args['basename'])) {
			$extension = substr($args['basename'], strrpos($args['basename'], '.'));
			$filename = substr($args['basename'], 0, strrpos($args['basename'], '.'));
			if(strpos($filename, '(')) {
				$filename = substr($filename,0,strpos($filename, '('));
			}
			$args['basename'] = $filename. '(' . $num++ . ')' . $extension;
		}
		copy($args['path'],$temp_dir.'/'.$args['basename']);*/
		$args['id'] = $args['id'].$args['basename'];
		$args['path'] = $temp_dir.'/'.$args['basename'];
		$args['status'] = true;

		// Note the file for later cleanup
		$_SESSION['plugins']['filesystem_attachments']['tmp_files'][] = $args['basename'];
