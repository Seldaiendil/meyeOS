            ...
            <html>
                ...
                <div id="progressbar" style="margin-top:60px;display:none;">
                    ...
                    <div id="wrapper" style="width:410px;border:1px solid black;height:18px;margin-left:10px">
                        <div id="internal" style="width:0px; background-repeat: repeat-x; height: 18px; background-image: url('eyeos/extern/images/bg_progress.png')"></div>
    ...
    public static function submitFile($path) {
        try {
            if(!isset($_FILES['Filedata'])) {
                echo '<div style="font-size:20px;font-family:Helvetica, Arial, Verdana, Sans, FreeSans;margin-top:80px;margin-right:15px;"><center>&nbsp;&nbsp;<img style="position:relative;top:15px"src="eyeos/extern//images/48x48/actions/dialog-close.png" />Error uploading files</center>';
                exit;
            }
            ...
            foreach($_FILES['Filedata']['name'] as $k=>$v) {
                if(!empty($v)) {
					...
					*/
					...
					$eventData = str_replace('"','\\"', json_encode(self::getFileInfo($destFile, $settings)));
					echo '<script>window.parent.eyeos.handleControlMessage(window.parent.qx.util.Json.parse("{\"__eyeos_specialControlMessage_header\":\"__control_enhancedData\",\"__eyeos_specialControlMessage_body\":{\"messages\":[{\"type\":\"file\",\"eventName\":\"uploadComplete\",\"eventData\":'.$eventData.'}],\"data\":\"null\"},\"__eyeos_specialControlMessage_options\":null}"));</script>';
...
//                    ClientMessageBusController::getInstance()->queueMessage($message);
			...
			echo '<div style="font-size:20px;font-family:Helvetica, Arial, Verdana, Sans, FreeSans;margin-top:80px;margin-right:15px;"><center>&nbsp;&nbsp;<img style="position:relative;top:15px"src="eyeos/extern//images/48x48/actions/mail-mark-notjunk.png" />Uploaded successfully</center>';
			exit;
        } catch (EyeException $e) {
            echo '<div style="font-size:20px;font-family:Helvetica, Arial, Verdana, Sans, FreeSans;margin-top:80px;margin-right:15px;"><center>&nbsp;&nbsp;<img style="position:relative;top:15px"src="eyeos/extern//images/48x48/actions/dialog-close.png" />Error uploading files: '.$e->getMessage().'</center>';
            exit;
        }
    }
