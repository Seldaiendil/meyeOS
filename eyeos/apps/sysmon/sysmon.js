...
qx.Class.define('eyeos.application.Sysmon', {
	...
	members: {
		...
		_drawGUI: function() {
			this._window = new eyeos.ui.Window(this, tr('System Monitor'), 'eyeos/extern//images/16x16/apps/utilities-system-monitor.png').set({
			...
			var refreshButton = new qx.ui.toolbar.Button(tr('Refresh'), 'eyeos/extern/images/22x22/actions/view-refresh.png').set({
				iconPosition: 'top'
			});
			refreshButton.addListener('execute', this._refreshProcessesList, this);
			toolbar.add(refreshButton, {column: 0, row: 0});
			
			var infoButton = new qx.ui.toolbar.Button(tr('Info'), 'eyeos/extern/images/22x22/status/dialog-information.png').set({
				iconPosition: 'top'
			});
			...
			var killButton = new qx.ui.form.Button(tr('Kill process'), 'eyeos/extern/images/22x22/actions/process-stop.png').set({
				iconPosition: 'left',
				margin: 4
			});
		...
		_refreshProcessesListCallback: function(procData) {
			...
			for(var i = 0; i < procData.length; i++) {
				rows.push(new Array(
						'<img src="eyeos/extern/images/16x16/mimetypes/application-x-executable.png" />',	//TODO display icon according to mimetype/extension
						procData[i].name,
						procData[i].pid,
