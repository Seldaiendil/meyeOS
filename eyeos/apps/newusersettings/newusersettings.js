...
qx.Class.define('eyeos.gui.form.RadioButton', {
	...
	members: {
		_addButton: function () {
			...
			var botton = new qx.ui.basic.Image('eyeos/extern/images/16x16/actions/window-close.png').set({
				maxWidth: 13,
				maxHeight: 13,
				minWidth: 13,
				maxWidth: 13,
...
qx.Class.define('eyeos.application.NewUserSettings', {
	...
	members: {
		init: function () {
			this.setWindow(new eyeos.ui.Window(this, 'User Settings', 'eyeos/extern//images/16x16/apps/accessories-text-editor.png').set({
				width: 660,
				height: 555,
				contentPadding: 0,
		...
		initValues: function() {
			...
			if (this.getUserImage() != null) {
				...
				this._inputImage.setSource('eyeos/extern/images/empty_profile.png');
			}
		},
...
//					mailBox.add(this._mailBoxAddMail);
					...
					this._listItem.addListener('changeSelection', function (e) {
						if (e.getData()[0] instanceof qx.ui.form.ListItem) {
							if (e.getData()[0].getModel().id == 'color') {
								...
								var path = e.getData()[0].getModel().path;
								eyeos.callMessage(this.getChecknum(), 'verifyPath', [path], function (results) {
									eyeos.callMessage(this.getChecknum(), '__FileSystem_browsePath', [path, null, null], function (results) {
										this._populateWallpapers(results);
									}, this);
								}, this);
							}
		...
		_populateWallpaper: function(result, path) {
			...
			var img;
			var postData;
			img = 'index.php?checknum='+this.getChecknum()+'&message=__FileSystem_getScaledImage&params[maxWidth]=50&params[maxHeight]=50&params[path]='+ result.absolutepath;
