...
class FileSystemExecModule implements IEyeosExecutableModule {
	 ...
	 * )
	 */
	 public static function scaleImage($params) {
	...
	/**
	 * Return the scaled version of image with headers
	 *
	 * @param Array $params = (
	 *		maxHeight => integer,		Max Height of output image
	 *		maxWidth => integer,		Max Width of output image
...
//		$response = MMapManager::getCurrentResponse();
		...
		$image = self::scaleImage($params);

		header('Content-Type: image');	//mime_content_type($image) don't work
	...
	/**
	 * Zip recursively one or more eyeOS items
	 * @param $params Array(
	 *		cwd ->	Current Working Directory, where to place the output files
	 *		files -> Can be eyeOS path or absolute path
	 *		output -> output name
	 * )
	 * @uses If the files have different root directory can produce erraticaly results
	 */
	public static function zip($params) {
		...
		if (!isset($params['cwd'])) {
			throw new EyeInvalidArgumentException('Error, $params[cwd] cannot be empty');
		}
		if (!is_array($params['files'])) {
			throw new EyeInvalidArgumentException('Error, $params[files] must be an array');
		}
		if (!isset($params['output'])) {
			throw new EyeInvalidArgumentException('Error, $params[output] cannot be empty');
		}
		try {
			//Check if the $params['output'] already exists
			$number = 1;
			$outputFile = FSI::getFile($params['cwd'].'/'.$params['output']);
			$theName = substr($outputFile->getName(), 0, strripos($outputFile->getName(), '.'));
			...
			while ($outputFile->exists()) {
				...
				$number++;
				$outputFile = FSI::getFile($params['cwd'].'/'.$nameForCheck);
			}

			$myFile = FSI::getFile($params['cwd']);
			$cwd = AdvancedPathLib::getPhpLocalHackPath($myFile->getRealFile()->getAbsolutePath());
			$localHackPath = AdvancedPathLib::getPhpLocalHackPath($outputFile->getRealFile()->getAbsolutePath());
			$cmd = 'cd '.escapeshellarg($cwd).';zip -ru '.escapeshellarg($localHackPath).' ';

			foreach ($params['files'] as $oneItem) {
				//This is for accept eyeOS files or absolute path
				$myFile = FSI::getFile($oneItem);
				if (method_exists ($myFile, 'getRealFile' ) ) {
					$fileName = AdvancedPathLib::getPhpLocalHackPath($myFile->getRealFile()->getAbsolutePath());
				} else {
					$fileName = $oneItem;
				}
				//Make relative path, and add . in the begining of the string
				$fileName = ".".str_replace($cwd,"", $fileName);
				$cmd .= escapeshellarg($fileName).' ';
			}
			shell_exec($cmd);

			$meta = MetaManager::getInstance()->getNewMetaDataInstance($outputFile);
            if($meta !== null) {
                $currentUser = ProcManager::getInstance()->getCurrentProcess()->getLoginContext()->getEyeosUser();
                ...
                $meta->set(self::METADATA_KEY_PERMISSIONS, AdvancedPathLib::permsToUnix(self::PERMISSIONS_MASK_FILE & ~$outputFile->getUMask()));
                $outputFile->setMeta($meta);
            }
            //notify listeners
            $outputFile->fireEvent('fileCreated', new FileEvent($outputFile));

			return "true";
		} catch (Exception $e) {

			Logger::getLogger('Ziping error: ')->error('Ziping error: '.$e);
			return "false";
		}
	/*
	 * If its necessary this is an implementation using php library (zipPHP), it's not finished between todo lines
	 * @param array $params (
	 *		'path' => array of strings with the paths
	 *		'zipArchive' => Object Zip
	 *		'zipDir' => the actual zipping folder
	 *	)
	 */
//		if (!isset($params['zipArchive'])) {
//			$zipArchive = new ZipArchive();
//			//@todo: filename ha de ser la carpeta base del parametre
...
//		} else {
//			//recuperem l'arxiu
//			$eyeFile = FSI::getFile($params['files']);
//			$realPath = AdvancedPathLib::getPhpLocalHackPath($eyeFile->getRealFile()->getAbsolutePath());
//
//			if($eyeFile->isDirectory()) {	//Si es directori, s'ha de tornar a iterar
//
//				$zipdir = $eyeFile->getName();
//				//es crea la carpeta
//				$zipArchive->addEmptyDir($zipdir);
//				//es torna a cridar la funció recorrent els seus fills
//				foreach ($params['files'] as $oneItem) {
//				/* TODO
//					$param = array (
//						'files' => $oneItem,
...
//				}
//
//			} else {	//Si es un arxiu s'afegeix a l'objecte zipArchive
//				$zipName = $eyeFile->getName();
//				if(isset($params['zipDir'])) {
//					$zipName = $params['zipDir'] . $zipName;
//				}
//				$zipArchive->addFile($realPath,$zipName);
//			}
//		}
	}
	 ...
	 * @param Array $params= (
	 *		cwd ->	Current Working Directory, where to place the output files
	 *		files -> array of strings with the eyeOSpaths of the zip files to unzip
	 * )
	 */
	 public static function unzip($params) {
		...
		try {
			...
			foreach ($params['files'] as $oneItem) {
				$cmd = '';
				// Creating a folder to put the unziped files
				$number = 1;
				$dirToCreate = FSI::getFile(substr($oneItem, 0, strripos($oneItem,'.')));
				$theName = $dirToCreate->getName();
				while ($dirToCreate->exists()) {
					$futureName = Array($theName, $number);
					$futureName = implode(' ', $futureName);
					$number++;
					$dirToCreate = FSI::getFile($params['cwd'] . '/' . $futureName);
				}
				$dirToCreate->mkdir();
				$inputFile = FSI::getFile($oneItem);
				$fileName = AdvancedPathLib::getPhpLocalHackPath($inputFile->getRealFile()->getAbsolutePath());
				//The file to unzip is in the parent directory, transforming absolute path into relative path
				$fileName = "..".str_replace($cwd,"", $fileName);
				$newDir = $cwd.'/'.$dirToCreate->getName();
				$cmd .= 'cd '.escapeshellarg($newDir).';unzip '.escapeshellarg($fileName).';';
				shell_exec($cmd);

				$dirIterator = new DirectoryIterator($newDir);
				foreach ($dirIterator as $fileinfo) {
					if (!$fileinfo->isDot()) {
						//Setting metadate for the uncompressed files
						$newRoute = $newDir .'/'. $fileinfo->getFilename();
						$realynewdir = $dirToCreate->getPath() . '/' . $fileinfo->getFilename();
						$newFile = FSI::getFile($realynewdir);
						$meta = MetaManager::getInstance()->getNewMetaDataInstance($newFile);
						if($meta !== null) {
							$currentUser = ProcManager::getInstance()->getCurrentProcess()->getLoginContext()->getEyeosUser();
							$group = ProcManager::getInstance()->getCurrentProcess()->getLoginContext()->getEyeosGroup();
							$meta->set(self::METADATA_KEY_OWNER, $currentUser->getName());
							...
							$meta->set(self::METADATA_KEY_CREATIONTIME, time());
							$meta->set(self::METADATA_KEY_MODIFICATIONTIME, time());
							$meta->set(self::METADATA_KEY_PERMISSIONS, AdvancedPathLib::permsToUnix(self::PERMISSIONS_MASK_FILE & ~$newFile->getUMask()));
			...
			Logger::getLogger('Unziping error: ')->error('Unziping error: '.$e);
			return "false";
		}
	 }
}
