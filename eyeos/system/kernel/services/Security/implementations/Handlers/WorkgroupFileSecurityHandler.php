 ...
 * @subpackage Security
 */
class WorkgroupFileSecurityHandler implements ISecurityHandler {
	/**
	...
	public function __construct(array $params = null) {
		if (self::$Logger === null) {
			self::$Logger = Logger::getLogger('system.services.Security.WorkgroupFileSecurityHandler');
		}
	}
	...
	public function checkPermission($object, IPermission $permission, LoginContext $context) {
		...
		if ($object instanceof EyeWorkgroupFile) {
			...
			//Searching if the user's privileges has changed in this file
			$metaOfFile = $object->getMeta();
			$metaOfFile = $object->getMeta();

			$rolesModified = $metaOfFile->get('roleModified');
			if($rolesModified){
				foreach ($rolesModified as $roleM){
					if($roleM['idUser'] == $eyeosUser->getId() ){
						switch ($roleM['newRole']){
							case WorkgroupConstants::ROLE_ADMIN:
								return true;
		...
		// Check access to a workgroup-conf:// file
