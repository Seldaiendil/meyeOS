	...
	public function getProcessByPid($pid) {
		...
		foreach($processTable as $process) {
			if ($process->getPid() == $pid) {
                ...
                /**
                 * If the process is not found, maybe session has been expired
                 * @link http://www.php.net/manual/en/session.configuration.php#ini.session.gc-maxlifetime
                 */
		throw new EyeProcException('Process $proc with PID ' . $pid . ' not found. Maybe session has been expired.');
	}
