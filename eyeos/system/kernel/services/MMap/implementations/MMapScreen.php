...
class MMapScreen extends Kernel implements IMMap {
	...
	protected function __construct() {
		self::$scripts = array(
			...
			EYE_ROOT . '/extern/js/eyeos/ui/window/Standard.js',
	...
	public function processRequest(MMapRequest $request, MMapResponse $response) {
		...
		if(SYSTEM_TYPE == 'release') {
			if(!file_exists(EYE_ROOT . '/' . EXTERN_DIR . '/js/basesystem.js')) {
				$qxjs = file_get_contents(EYE_ROOT . '/' . EXTERN_DIR . '/js/qx.js');
				$eyeosjs = file_get_contents(EYE_ROOT . '/' . EXTERN_DIR . '/js/eyeos.compressed.js');
				$data = $qxjs . "\n" . $eyeosjs;
				...
				file_put_contents(EYE_ROOT . '/' . EXTERN_DIR . '/js/basesystem.js', JSMin::minify($data));
			}
		}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<link rel="icon" type="image/png" href="eyeos/extern/images/favicon.png" />';
                if(SYSTEM_TYPE != 'release') {
                    $body .= '<script type="text/javascript" src="eyeos/extern/js/eyeos.js">';
					$body .= '</script>
					...
					$body .= '<script src="eyeos/extern/js/basesystem.js"></script>';
				}
