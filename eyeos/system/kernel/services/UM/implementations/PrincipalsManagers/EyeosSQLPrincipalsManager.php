...
class EyeosSQLPrincipalsManager extends Kernel implements IPrincipalsManager {
	const WORKGROUP_VALIDATION_REGEXP = '[a-zA-Z0-9ÑñáéíóúÁÉÍÓÚàèìòùÀÈÌÒÙüÜçÇñÑäëïöüÜÏâêîôûÂÊÎÔÛ][a-zA-Z0-9ÑñáéíóúÁÉÍÓÚàèìòùÀÈÌÒÙüÜçÇñÑäëïöüÜÏâêîôûÂÊÎÔÛ ·.\-_]{2,40}';
	const USERNAME_VALIDATION_REGEXP = '[a-zA-Z0-9][a-zA-Z0-9.\-_]{2,40}';
	...
	public static function checkPrincipalNameUser($principalName) {
		if (!is_string($principalName)) {
			throw new EyeInvalidPrincipalNameException('$principalName must be a string.');
		}
		if (preg_match('/^' . self::USERNAME_VALIDATION_REGEXP . '$/i', $principalName) === 1) {
			return;
		}
	...
	public static function checkPrincipalNameWorkgroup($principalName) {
		if (!is_string($principalName)) {
			throw new EyeInvalidPrincipalNameException('$principalName must be a string.');
		}
		if (preg_match('/^' . self::WORKGROUP_VALIDATION_REGEXP . '$/i', $principalName) === 1) {
			return;
		}
		throw new EyeInvalidPrincipalNameException('"' . $principalName . '" is not a valid name (valid characters: [a-z 0-9 - _], min. length: 2, max length: 40)');
	}
	...
	public function createGroup(IGroup $group) {
		try {
			...
			self::checkPrincipalNameWorkgroup($group->getName());
	...
	public function createUser(IUser $user) {
		...
		self::checkPrincipalNameUser($user->getName());
	...
	public function createWorkgroup(IWorkgroup $workgroup) {
		...
		self::checkPrincipalNameWorkgroup($workgroup->getName());
