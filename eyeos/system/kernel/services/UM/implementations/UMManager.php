...
class UMManager extends Kernel implements IUM, IPrincipalsManager, IObservableUMManager {
	...
	public static function getEyeosUserDirectory($user) {
		...
		if(!realpath(USERS_PATH . '/' . utf8_basename($username))) {
			if(!utf8_basename($username)) {
				throw new EyeInvalidArgumentException('Error getting eyeOS user directory.');
			}
		} else {
			if(realpath(USERS_PATH) == realpath(USERS_PATH . '/' . utf8_basename($username)) || 
					strpos(realpath(USERS_PATH . '/' . utf8_basename($username)), realpath(USERS_PATH)) !== 0) {
				throw new EyeInvalidArgumentException('Error getting eyeOS user directory.');
			}
		}
	...
	public static function getEyeosWorkgroupDirectory($workgroup) {
		...
		if(!realpath(WORKGROUPS_PATH . '/' . utf8_basename($workgroupName))) {
			if(!utf8_basename($workgroupName)) {
				throw new EyeInvalidArgumentException('Error getting eyeOS workgroup directory.');
			}		
		} else {
			if(realpath(WORKGROUPS_PATH) == realpath(WORKGROUPS_PATH . '/' . utf8_basename($workgroupName)) || 
					strpos(realpath(WORKGROUPS_PATH . '/' . utf8_basename($workgroupName)), realpath(WORKGROUPS_PATH)) !== 0) {
				throw new EyeInvalidArgumentException('Error getting eyeOS workgroup directory.');
			}
		}
